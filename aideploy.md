...
**问询呈现**：优先使用你运行环境自带的交互式提问机制（如 Claude Code 的 `AskUserQuestion`、其他 agent 的表单/选项 UI）——按可点选的选项卡片出题并给默认项，别让用户手打整句答案；无此类工具才退化为纯文本编号列点。单轮问题数以机制上限为准（如 4 个），装不下的问题用默认值，在方案卡里标注「默认采用，可改」。
